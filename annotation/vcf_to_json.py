...
for v in vcf:
    for alt in v.alts:
        ...
        if 'CSQ' in variant['info']:
            ...
            # delete all freq info
            # for each transcript
            for trans in variant['CSQ']:
                keys=variant['CSQ'][trans].keys()
                for k in keys:
                    if 'ExAC' in k:
                        variant[k]=variant['CSQ'][trans][k]
                        del variant['CSQ'][trans][k]
                    if 'MAF' in k:
                        variant[k]=variant['CSQ'][trans][k]
                        del variant['CSQ'][trans][k]
            variant['Existing_variation']=[variant['CSQ'][trans]['Existing_variation'] for trans in variant['CSQ']]
            for trans in variant['CSQ']: del variant['CSQ'][trans]['Existing_variation']
            #print(variant)
        ...
        variant['MISS_COUNT']=len([s for s in v.samples if v.samples[s]['GT'].count(None)==2])
        variant['WT_COUNT']=len([s for s in v.samples if v.samples[s]['GT'].count(variant['ref'])==2])
        ...
        variant['HET']=dict([(s,1,) for s in v.samples if v.samples[s]['GT'].count(variant['alt'])==1])
        variant['HOM']=dict([(s,1,) for s in v.samples if v.samples[s]['GT'].count(variant['alt'])==2])
        ...
        #variant['HET_COUNT']=len(variant['HET'])
        #variant['HOM_COUNT']=len(variant['HOM'])
        #print(v.info.keys())
        #print(variant)
        ...
        for k in [k for k in variant if k.startswith('EXAC') or k=='AJcases' or k=='UCLEX' or k=='CADD']:
            val=variant[k]
            for x in val.split(','):
                a, n, = x.split(':')
                a=a.split('>')[1]
        ...
        for k in variant:
            try:
                variant[k]=float(variant[k])
            except:
                continue
        #for s in v.samples: variant[s]=v.samples[s]['GT'].count(variant['alt'])
        #print('variants', dumps(variant))
        print(dumps(variant))
