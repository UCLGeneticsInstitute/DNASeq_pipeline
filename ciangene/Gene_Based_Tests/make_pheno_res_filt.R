...
names<- gsub(basename(kinDecom),pattern="_.*",replacement="") 
write.table(names,paste0(dir,"tk_depth_filt_groups"),col.names=F,row.names=F,quote=F,sep="\t") 
for(i in 1:length(kinDecom))
{
	file<-read.table(kinDecom[i],header=T, sep="\t") 
	if(i==1)
...
colnames(mat)<-c(rep("Pheno",2),names)
write.table(mat,paste0(dir,"/tk_depth_phenotype_file") ,col.names=F,row.names=F,quote=F,sep="\t") 
