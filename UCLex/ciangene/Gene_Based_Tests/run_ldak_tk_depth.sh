...
Groups=$bDir"Res-pheno-file-order"
...
templateScript=/cluster/project8/vyp/cian/data/UCLex/ciangene/scripts/LDAK/ldak_tk_depth_template.sh
variants=$bDir"/Clean_variants_func_rare"

phenotype=$bDir"/tk_depth_phenotype_file"
oFolder=$bDir"/LDAK_gene_tests_all_phenos_new/"
if [ ! -e $oFolder ]; then mkdir $oFolder; fi

for pheno in $(seq 1 22) 
...
do
	...
	do
		...
		do
			oFile=LDAK_${batch}_${maf}_${missing}.tk.depth.sh
